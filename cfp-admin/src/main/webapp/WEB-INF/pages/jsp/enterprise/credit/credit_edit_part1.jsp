...
<html>
<body>
<div id="credit_edit_part1" class="container-fluid" style="padding: 50px 0px 0px 10px;width:50%;">
<input style="float: right;width: 100px" type="button" class="btn  btn-primary"  value="保 存" onclick="save_credit_edit_part1();">
<form class="form-horizontal" id="credit_edit_part1_form" method="post">
          ...
          <td>
                       	...
                       	<select name="province" id="province" style="width: 100px">
                        	...
                        	<c:if test="${not empty provinceInfos}">
	                        	<c:forEach items="${provinceInfos}" var="provinceInfo">
	                        		<c:if test="${provinceInfo.provinceId == credit.province}">
                        ...
                        <select name="city" id="city" style="width: 100px">
                        	...
                        	<c:if test="${not empty bornCitys}">
	                        	<c:forEach items="${bornCitys}" var="bornCity">
	                        		<c:if test="${bornCity.cityId == credit.city}">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        style="width: 440px; height: 100px;">${credit.useDescription }</textarea>
                    </div>
                </div>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        style="width: 440px; height: 100px;">${credit.projectDescription }</textarea>
                    </div>
                </div>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        class="easyui-validatebox" validType="length[0,200]"
                        style="width: 440px; height: 100px;">${credit.riskControlInformation }</textarea>
                    </div>
                </div>
...
//省份下拉框。
$("#credit_edit_part1_form #province").combobox({
    textField: 'PROVINCENAME',
    valueField: 'PROVINCEID',
    onSelect: function (record) {
        $("#credit_edit_part1_form #city").combobox("reload",
        	'${ctx}/jsp/constant/loadCity?pCityId=0&provinceId=' + record.PROVINCEID + '&selectedDisplay=selected');
    }
});

//城市下拉框。
$("#credit_edit_part1_form #city").combobox({
    textField: 'CITYNAME',
    valueField: 'CITYID'
});

//借款用途下拉框。
$("#credit_edit_part1_form #loanUseage").combobox({
    ...
    onLoadSuccess: function (record) {
    	$("#credit_edit_part1_form #loanUseage").combobox("select", '${loan.loanUseage}');
    }
});

//内部评级下拉框。
$("#credit_edit_part1_form #internalRating").combobox({
    url: '${ctx}/jsp/constant/loadSelect?constantTypeCode=internalRating&parentConstant=0&selectedDisplay=selected',
    textField: 'CONSTANTNAME',
    valueField: 'CONSTANTVALUE',
    onLoadSuccess: function (record) {
    	$("#credit_edit_part1_form #internalRating").combobox("select", '${credit.internalRating}');
    }
});

//借款产品下拉框。
$("#credit_edit_part1_form #loanProductId").combobox({
    url: '${ctx}/jsp/product/loan/loadLoanProduct?selectedDisplay=selected',
    textField: 'PRODUCTNAME',
    valueField: 'LOANPRODUCTID',
    onLoadSuccess: function (record) {
    	$("#credit_edit_part1_form #loanProductId").combobox("select", '${loan.loanProductId}');
...
//打款卡下拉框。
$("#credit_edit_part1_form #inCardId").combobox({
    url: '${ctx}/jsp/enterprise/loadCard?enterpriseId=${enterpriseId}&selectedDisplay=selected',
    textField: 'CARDCODE',
    valueField: 'CARDID',
    onLoadSuccess: function (record) {
    	$("#credit_edit_part1_form #inCardId").combobox("select", '${loan.inCardId}');
    }
});

// 执行:保存。
function save_credit_edit_part1(){
	
	// 验证
	if(!$("#credit_edit_part1_form").form('validate')){
		return false;
	}
	...
	$.post('${ctx}/jsp/enterprise/loan/save_enterprise_loan_credit?r=' + Math.random(),
   		$("#credit_edit_part1_form").serialize(),
   		function(data){
   	    	if(data.result == 'success'){
