...
<div style="padding: 10px">
	<div style="float: left;margin-left: 10px;">
		<c:if test="${actionType == 'add'}">
			<span style="font-size: 16px;font-weight: bold;">新增企业标</span>
		</c:if>
		<c:if test="${actionType == 'edit'}">
			<span style="font-size: 16px;font-weight: bold;">编辑企业标</span>
		</c:if>
	</div>
	...
	<div id="main" style="display: block;" data-options="closable:false,collapsible:false,minimizable:false,maximizable:false">
		...
		<div id="detail" class="container-fluid" style="padding: 5px 0px 0px 10px">
			<div id="tt" class="easyui-tabs" style="width:auto;height:650px" >
				...
				<c:if test="${actionType == 'add'}">
					<div title="新增企业标项目" style="padding:10px;" data-options="href:'${ctx}/jsp/enterprise/loan/to_pledge_add_part1?loanApplicationId=${loanApplicationId }'">1</div>
					<div title="相关附件" style="padding:10px;" data-options="href:'${ctx}/jsp/enterprise/loan/to_pledge_add_part2?loanApplicationId=${loanApplicationId }'">2</div>
				</c:if>
				
				<c:if test="${actionType == 'edit'}">
					<div title="编辑企业标项目" style="padding:10px;" data-options="href:'${ctx}/jsp/enterprise/loan/to_pledge_edit_part1?loanApplicationId=${loanApplicationId }'">1</div>
					<c:if test="${applicationState == 0 }">
						<div title="相关附件" style="padding:10px;" data-options="href:'${ctx}/jsp/enterprise/loan/to_pledge_edit_part2?loanApplicationId=${loanApplicationId }'">2</div>
					</c:if>
				</c:if>
