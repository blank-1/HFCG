	...
	<div id="financialPlanList_toolbar" style="height:auto">
        <div class="container-fluid" style="padding: 5px 0px 0px 10px">
            <form action="${ctx}/jsp/lendList/financialPlanList" method="post" class="form-inline">
            	...
            	<span>购买人用户名:</span><input type="text" id="searchLeanUserName" size=6/>
            	...
            	<!-- <a href="#" onclick="javascript:goto('${ctx}/jsp/loanManage/loan/toLoanApplicationList')">借款列表</a> -->
            </form>
        </div>
    </div>
...
<script language="javascript">
    ...
    function loadList(){
        $("#financialPlanList_list").datagrid({
            idField: 'lendOrderId',
            title: '所有省心列表',
            url: '${ctx}/jsp/lendList/financialPlanList',
            pagination: true,
            pageSize: 10,
                      ...
                      */
                      ...
                      {field:'action',align:'left',title:'操作',width:100,
                          formatter:function(value,row,index){
                              ...
                              value += '<a class="label label-important" onclick="reCreateAgreement('+index+')">生成合同</a> &nbsp;&nbsp;';
                              return value;
                          }
                      }
            ]]
    ...
    function reCreateAgreement(index) {
    	$("#financialPlanList_list").datagrid("selectRow",index);
    	var selRow = $("#financialPlanList_list").datagrid("getSelected");
        Utils.loading();
    	...
    	if (selRow) {
    		$.ajax({
                url:"${ctx}/jsp/lendList/tofinancialPlanRecreateAgreement?lendOrderId="+selRow.lendOrderId+"&r=" + Math.random() ,
                type:"post",
                data:{},
                async:false,
                success:function(data){
                  if(data=='success')
                    $.messager.alert('提示', '省心计划合同生成成功!', 'info');
                  else
                    $.messager.alert('提示','省心计划合同生成失败!', 'info');
                }
              });
    	}
    	Utils.loaded();
