    ...
    <div class="control-group">
        ...
        <div class="controls">
        	<c:if test="${loanType == 0 || loanType == 1 || loanType == 7}">
        		<c:if test="${state == 0}">身份证</c:if>
	            <c:if test="${state == 1}">工资流水</c:if>
	            <c:if test="${state == 2}">个人信用报告</c:if>
	            <c:if test="${state == 3}">结婚证 </c:if>
	            <c:if test="${state == 4}">社保截图 </c:if>
	            <c:if test="${state == 5}">收入证明</c:if>
	            <c:if test="${state == 6}">房产证</c:if>
	            <c:if test="${state == 7}">土地证</c:if>
        	</c:if>
        	...
        	<c:if test="${loanType == 8}">
        		<c:if test="${state == 0}">身份证</c:if>
        		<c:if test="${state == 10}">合同协议</c:if>
        		<c:if test="${state == 2}">车辆照片</c:if>
        		<c:if test="${state == 3}">证件相关</c:if>
        	</c:if>
		<c:if test="${loanType == 9}">
        		<c:if test="${state == 0}">[现金贷]债权清单</c:if>
        		<c:if test="${state == 1}">[现金贷]其它</c:if>
        	</c:if>
        </div>
    </div>
    <div class="control-group">
...
<script language="javascript" >
    ...
    $("#upload_snapshot_add_form").form({
        ...
        url:'${ctx}/jsp/loanManage/loan/saveUploadSnapshot',
