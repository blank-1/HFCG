...
public class ReflectionUtil {
     ...
     * 调用Setter方法.使用value的Class来查找Setter方法.
     *
     * @see #invokeSetterMethod(Object, String, Object)
     */
    public static void invokeSetterMethodIfExists(Object target, String propertyName, Object value) {
        try {
            invokeSetterMethod(target, propertyName, value, null);
        } catch (Exception e) {
            logger.warn("没有此get方法:" + propertyName);
        }
    }
     ...
     * 直接读取对象属性值, 无视private/protected修饰符, 不经过getter函数.
     */
    public static Object getFieldValueIfExists(final Object object, final String fieldName) {
        Field field = getDeclaredField(object, fieldName);
    ...
    public static List<String> getFiledName(Object o) {
        try {
            ...
            for (int i = 0; i < fields.length; i++) {
                if (!fields[i].getGenericType().toString().equals("boolean") && fields[i].getAnnotation(AllowNull.class) == null)
    ...
    /**
     * 获取所有field,包含父类，但是field 名称不能重，取值原则：就近原则（子类的）
     *
     * @param clazz  要处理的类class
     * @param result 接收容器
     */
    public static void allAccessibleFields(Class<?> clazz, Map<String, Field> result) {
        ...
        if (clazz.getSuperclass() != Object.class) {
            allAccessibleFields(clazz.getSuperclass(), result);
        }
    }
     ...
     * 获取所有field,包含父类，但是field 名称不能重，取值原则：就近原则（子类的）
     *
     * @param dataSource 要处理的类对像
     * @see #allAccessibleFields(Class, Map)
     */
    public static Map<String, Field> allAccessibleFields(Object dataSource) {
        return allAccessibleFields(dataSource.getClass());
    }
     ...
     * @see #allAccessibleFields(Class, Map)
     */
    public static Map<String, Field> allAccessibleFields(Class<?> clazz) {
        Map<String, Field> result = new HashMap<>();
        allAccessibleFields(clazz, result);
        return result;
    }
}