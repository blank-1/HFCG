...
public class RequestUtil {
    ...
    public static String JSONString(HttpServletRequest request, String... ignoreKeys) {
        ...
        while (parameterNames.hasMoreElements()) {
            ...
            if (ignoreKeys != null && ignoreKeys.length > 0) {
                if (Arrays.binarySearch(ignoreKeys, name) > -1) {
                    continue;
                }
            }
            String value = request.getParameter(name);
            sb.append(String.format("\"%s\":\"%s\"", name, StringUtils.isEmpty(value) ? "" : value));
    ...
    public static String JSONArray(HttpServletRequest request, String... ignoreKeys) {
        if (ignoreKeys != null && ignoreKeys.length > 0) {
            Arrays.sort(ignoreKeys);
        }
        ...
        while (parameterNames.hasMoreElements()) {
            ...
            if (ignoreKeys != null && ignoreKeys.length > 0) {
                if (Arrays.binarySearch(ignoreKeys, key) > -1) {
                    continue;
                }
            }
            String[] value = request.getParameterValues(key);

            sb.append(String.format("\"%s\":\"%s\"", key, value == null || value.length == 0 ? "[]" : Arrays.toString(value)));
            if (parameterNames.hasMoreElements()) {
                sb.append(",");
            }
        }
