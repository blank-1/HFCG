...
//        throw new SecurityException("手机号码格式错误");
     ...
     * @return true 有非法字符
     */
    public static boolean checkSQLStr(String str){
    ...
    public static boolean checkSQLStrForSimple(String str){
    	boolean result=false;
    	String inj_str = "and:exec:insert:select:delete:update:count:*:chr:mid:master:truncate:char:declare:script:alert";
        String inj_stra[] = inj_str.split(":");
        for (int i = 0; i < inj_stra.length; i++) {
            if (str.indexOf(inj_stra[i])!=-1) {
            	result = true;
    ...
    public static boolean checkStrForXSS(String str){
    	boolean result=false;
    	String inj_str = "alert:<:>:script:document:cookie:'";
        String inj_stra[] = inj_str.split(":");
        for (int i = 0; i < inj_stra.length; i++) {
            if (str.indexOf(inj_stra[i])!=-1) {
            	result = true;
