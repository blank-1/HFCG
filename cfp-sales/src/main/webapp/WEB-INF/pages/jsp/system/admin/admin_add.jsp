...
<form class="form-horizontal" id="admin_addAdmin_form" method="post">
	...
	<input type="hidden" name="adminCode" id="adminId" value="${adminInfo.adminCode}"/>
	<!-- for 员工匹配角色 -->
            ...
            <div class="controls">
                <input type="text" style="width: 200px"
                 class="easyui-validatebox" required="true" missingMessage="工号不能为空"
                       name="loginName" id="loginName" value="${adminInfo.loginName}">
            </div>
        </div>
    ...
    <div class="control-group">
	    ...
	    <div class="controls">
	    	<div id="dept_toolbar" style="height:auto">
		        <a id="role_add" href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="appendIcon();">新增</a>
		        <a id="role_del" style="display: none;" href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="removeIcon();">删除</a>
		    </div>
    ...
    <div class="control-group" id="organize">
        <label class="control-label">组织机构<span style="color: red">*</span></label>
        <div class="controls">
            <div id="">
                <table id="showOrganizeTree" style="width:200px;">
                    <thead>
                    <th data-options="field:'name'" width="auto">组织机构&nbsp;</th>
                    </thead>
                </table>
            </div>
        </div>
...
<script language="javascript" >
    ...
    $('#showOrganizeTree')
            .treegrid(
            {
                ...
                onLoadSuccess:function(){
                    <c:if test="${add eq 'false'}">
                        $('#showOrganizeTree').treegrid('select',${adminOrganizetion.organizeId});
                    </c:if>
                }
            });
    ...
    $("#admin_addAdmin_form").form({
        ...
        onSubmit:function() {
            ...
            if(result){
                if(row==null) {
                    $.messager.alert("系统提示", "请选择组织机构!", "info");
                    return false;
                }else{
                    $("#organizeId").val(row.id);
                    return true;
                }
            }
            return false;
