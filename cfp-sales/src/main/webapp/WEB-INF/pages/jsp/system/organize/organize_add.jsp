...
<form class="form-horizontal" id="function_saveFunction_form" method="post">
    ...
    <input type="hidden" name="organizeId" id="functionId" value="${functionId}"/>
    <input type="hidden" name="parentId" id="pFunctionId" value="${pFunctionId}"/>
    ...
    <div class="control-group">
        <label class="control-label">组织机构编码<span style="color: red">*</span></label>
        <div class="controls">
            <input type="text" style="width: 200px" readonly
            	class="easyui-validatebox"
                   name="organizeCode" id="organizeCode" value="${code}" />
        </div>
    </div>
    
    <div class="control-group">
        <label class="control-label">组织机构名称<span style="color: white">*</span></label>
        <div class="controls">
			<input type="text" style="width: 200px"
				   class="easyui-validatebox"
				   name="organizeName" id="organizeName" value="${functionInfo.organizeName}" />
        </div>
    </div>

	<div class="control-group">
		<label class="control-label">相关属性</label>
		<%--<div class="controls-text" id="functionCode" style="width:500px;">${functionInfo.organizeName}&nbsp;</div>--%>
	</div>

	<div class="control-group">
		<label class="control-label">描述</label>
		 <textarea name="description" autocomplete="off" class="textbox-text" placeholder=""
				   style="margin-left: 0px; margin-right: 0px; height: 75px; width: 350px;">${functionInfo.description}</textarea>
	</div>
...
<script language="javascript" >
    ...
    $("#function_saveFunction_form").form({
        url:'${ctx}/jsp/system/organize/saveOrganize',
        onSubmit:function() {
        	 endEditing();
             //遮罩
             Utils.loading(); 
        ...
        success:function(data) {
            ...
            var editingId = parent.$("#showOrganizeTree").treegrid("getSelected").id;
            data = eval('(' + data + ')');
            if(data.name == 'null') {
            	returnMsg();
            ...
            if(data.success) {
                   ...
                   //alert(transId);
            	   parent.$("#showOrganizeTree").treegrid("reload");//刷新树结构
            	   parent.$("#showOrganizeTree").treegrid({
        				onLoadSuccess : function() {
        	            	   parent.$("#showOrganizeTree").treegrid("select",transId);
        	            	   parent.$("#showOrganize").tabs('update', {
        		           			tab: $("#showOrganize").tabs('getTab',"组织机构信息"),
        		           			options: {
        		           				href: "${ctx}/jsp/system/organize/to_organize_show?functionId="+transId
        		           			}
        		           	   });
        		           	   parent.$("#showOrganize").tabs('select',"组织机构信息");
        				}
        		   });
