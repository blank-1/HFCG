...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${ctx}/css/base.css"/><!-- index css -->
    ...
    <link rel="stylesheet" type="text/css" href="${ctx}/css/w_style.css"/>
</head>
        ...
        <div class="loanlist-con">
            ...
            <div class="loanlist-bot">
                <div class="loanlist-bot-le">
                    <div class="loanlist-bot-le-tp money">
                        <div class="loanlist-bot-div">预期年化收益：<br/>
                            <big class="bigyear"
                                 data-pay="${loanApplicationListVO.annualRate}">${loanApplicationListVO.annualRate}
                                <small>%</small>
                        ...
                        <div class="loanlist-bot-div lb-br-none">借款金额：<br/>
                            <big class="bigyear" id="confirmBalance"><fmt:formatNumber
                                    value="${loanApplicationListVO.confirmBalance}" pattern="#,#00"/> </big>元
                            <div class="lb-jdt">
                    ...
                    <div class="loanlist-bot-le-bt">
                        <div class="loanlist-bot-le-bt-le">
                            <p><i class="list-bot-icon1"></i>起投金额：<i class="list-bot-word"
                        ...
                        <div class="loanlist-bot-le-bt-ri">
                            <c:if test="${not empty guaranteeCompany}">
                                <p><i class="list-bot-icon3"></i>担保公司：<i
                                        class="list-bot-word">${guaranteeCompany.companyName}</i></p>
                            </c:if>
                ...
                <c:if test="${loanPublish.publishRule eq '2'}">
                    <div class="loanlist-bot-ri" id="only_automatic">
                        <div class="w_SaveWorry">
                            <h2>省心计划用户专享</h2>
                            <button class="prior" onclick="showPeacePlan();">我要参与</button>
                            <p>参与省心计划，帮您投资理财更省心。</p>
                        </div>
                    </div>
                </c:if>
                <!-- 仅省心（结束） -->
                ...
                <c:if test="${loanPublish.publishRule eq '1'}">
                    <div class="loanlist-bot-ri" id="count_second"
                         <c:if test="${second==0}">style="display: none;"</c:if>>
                        <div class="w_SaveWorry">
                            <h2>省心计划优先投资中，请您稍候...</h2>
                            <p>预计优先结束时间： <span class="timeOut">${second }</span>秒</p>
                            <button class="prior" onclick="showPeacePlan();">我要优先</button>
                            <p>参与省心计划，帮您投资理财更省心。</p>
                        </div>
                    </div>
                </c:if>
                ...
                <c:if test="${loanPublish.publishRule ne '2'}">
                    <div class="loanlist-bot-ri" id="count_end" <c:if test="${second!=0}">style="display: none;"</c:if>>
                        <!--水印图 预热中-->
                        <c:if test="${loanApplicationListVO.begin eq'false'}"><b id="yure"
                                                                                 class="biao biao5"></b></c:if>
                        <!--水印图 已结清-->
                        <c:if test="${loanApplicationListVO.applicationState eq '7'||loanApplicationListVO.applicationState eq '8'}"><b
                                id="biao" class="biao biao4"></b></c:if>
                        <!--水印图 满标-->
                        <c:if test="${loanApplicationListVO.applicationState eq '4'||loanApplicationListVO.applicationState eq '5'}"><b
                                id="biao" class="biao biao3"></b></c:if>
                        <!--水印图 还款中-->
                        <c:if test="${loanApplicationListVO.applicationState eq '6'}"><b id="biao"
                                                                                         class="biao biao2"></b></c:if>

                        <div class="clear_20"></div>
                        <p>剩余金额：<span id="mony2" data-money="${loanApplicationListVO.remain}">
							<fmt:formatNumber value="${loanApplicationListVO.remain}" pattern="#,##0.00"/></span><span
                                id="jiliang">元</span><i>（每人限投：<font id="limited">
                            <fmt:formatNumber value="${loanApplicationListVO.maxBuyBalanceNow}"
                                              pattern="#,##0.00"/></font><span>元</span>）</i></p>
                        ...
                        <c:if test="${not empty sessionScope.currentUser}">
                            <p class="mt-10">
                                <span class="list-cfq">财富劵：<font>${useageSum}</font>元 <i id="list-cfq-icon-i"
                                                                                         class="shang"></i><em>(${useageCount}张)</em></span>
                            </p>
                        </c:if>
                        <div class="clear"></div>
                            ...
                            <div class="list-cfq-je">
                                <div class="list-cfq-img">
                                    <i class="list-img-ile" style="float:left;"><img
                                            src="${ctx }/images/news_icon/le.png"/></i>
                                    <i class="list-img-iri" style="float:right;"><img
                                ...
                                <p class="list-p-bor-none"><i><a href="http://help.caifupad.com/guide/caifuquan/">如何获取财富券？</a></i>
                                </p>
                                <div class="clear"></div>
                            </div>
                            <div class="clear"></div>
                        ...
                        <p class="mt-20">
							<span class="list-inpit">
								<input type="text" id="money" style="margin-bottom:5px;" placeholder="请输入投标金额"
                                       maxlength="10" class="ipt-input"/><em class="yuan1">元</em>
							</span>
                        ...
                        <div class="clear"></div>
                        <p id="errorMoneyInfo" class="list-tip hui"><img src="../images/fdetail/dingpai.png"/>实际支付金额满5000元，奖5元财富券
                        </p>
                        <div class="clear"></div>
                        <p class="mt-20">
								<span class="list-toub">
									...
									<button type="button" id="allbalance" class="btn-small"
                                            <c:if test="${empty sessionScope.currentUser}">data-mask='mask'
                                            data-id="login"</c:if>>全部余额</button>
								</span>
...
<div class="list_tab">
    ...
    <div class="block">
        ...
        <div class="list-box-main">
            ...
            <dl>
                <dd>借款用途： <customUI:dictionaryTable constantTypeCode="loanUseage" desc="true"
                                                    key="${loanApplicationListVO.loanUseage}"/></dd>
        ...
        <div class="list-box-main">
            ...
            <c:if test="${loanApplicationListVO.loanType ne '0' && loanApplicationListVO.loanType ne '8'&& loanApplicationListVO.loanType ne '9' }">
    ...
    <div class="div">
        <div class="repayment-list">
            <p>已还本息：<span class="c-orange"><fmt:formatNumber value="${hasPaidBalance}" pattern="#,##0.00"/>元</span>
                待还本息：<span class="c-orange"><fmt:formatNumber value="${waitPaidBalance}" pattern="#,##0.00"/>元</span>
                (待还本息因算法不同可能会存误差，实际金额以到账金额为准！)</p>
...
<div class="masklayer masklback" id="cjd">
    ...
    <!-- equity start -->
    <div class="flbuy">
        <p class="clearFloat"><span class="fsp1">借款名称：</span> <span class="fsp2"
                                                                    id="title">${loanApplicationListVO.loanApplicationTitle}</span>
        </p>
        <p class="clearFloat"><span class="fsp1">投标金额：</span> <span class="fsp2"><big class="c_red"><i id="buymoney">10,000.00</i></big>元</span>
        </p>
        <p class="clearFloat"><span class="fsp1">预期收益：</span> <span class="fsp2"><small id="expected" class="c_red">300.05</small>元</span>
        </p>
        <p class="clearFloat"><span class="fsp1">账户余额：</span> <span class="fsp2"><small id="account"><fmt:formatNumber
        ...
        <form action="${ctx}/finance/bidLoanByAccountBalance" id="lendForm" method="post" name="form" class=" mt-20">
            ...
            <div class="input-group">
                <label for="checkBox">
                    <input type="checkbox" name="checkBox" id="checkBox" checked class="mr-5 ml-90 ipt-check"/><a
                        href="javascript:" class="ahui" data-mask="mask" data-id="serverProtocol">《出借咨询与服务协议》</a>
                </label>
                <em class=" ml-90"></em>
            </div>
            <div class="groupbtn clearFloat">
                ...
                <label>
                    <button type="button" id="otherPay" class="btn btn-write ml-20 mt-0 help-tips"
                            style="cursor: pointer;">使用其它方式支付<p><img src="${ctx}/images/cfq_icon.jpg"/>戳我，戳我，使用优惠券吧~</p>
                    </button>
                </label>
            </div>
        </form>
    ...
    $(function () {
        ...
        if (second > 0) {
            var timer = setInterval(function () {
                $("#count_second").find("span[class='timeOut']").html(second);
                second--;
                if (second == 0) {
                    setTimeout(function () {
                        clearInterval(timer);
...
<script type="text/javascript">
    ...
    j(function () {
        ...
        j(document).on("click", "#viewerFrame>div img", function () {
            ...
            // --e
            j("#bigpicture").css("z-index", "9999").slideDown(500);
        });
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < d_rows.length; i++) {
                    ...
                    var sx = "";
                    if (data.loType == 1) {
                        sx = "<a href='https://www.caifupad.com/zt/peacePlan/'>省心</a>";
                    }
                    thtml += "<ul>";
                    thtml += "<li class='w_worry'>" + data.lenderName + sx + "</li>";
    ...
    //跳转到省心计划专题页
    function showPeacePlan() {
        location.href = '${ctx}/finance/list?tab=zhe'
    }
</script>
<!--借款详情end-->
<!-- tabdiv start -->
    ...
    <c:if test="${not empty sessionScope.currentUser}">
        <div class="bay-window" id="bay-windows">
          ...
          <span class="bay-input"><input type="text" id="bay_ipt_money"
                                         <c:if test="${loanApplicationListVO.begin eq'false' || loanApplicationListVO.applicationState ne '3'}">class="iptdis"
                                         disabled </c:if>/><i class="bay-yuan" id="bay-yuan">元</i></span>
