...
<div class="wrapper">
 ...
 /*  <c:if test="${loanApplicationListVO.begin eq'false'||loanApplicationListVO.applicationState ne '3'}">
      $(".loanlist-bot-ri input.ipt-input[type=text]:visible,.loanlist-bot-ri button.btn").each(function() {
...
<div class="masklayer masklback" id="ed_ex_psd">
		...
		<input value="${not empty sessionScope.currentUser}" type="hidden" id="isLogin"/>
		<input value="${loanApplicationListVO.begin eq'true'}" type="hidden" id="isTrue"/>
</div>
<!-- 找回交易密码-结束 -->
...
function myInterval(){
	$("#piptime").attr("disabled",false);
	$("#piptime2").attr("disabled",false);
	$("#piptime3").attr("disabled",false);
	$("#piptime4").attr("disabled",false);
	$("#piptime5").attr("disabled",false);
	$("#piptime6").attr("disabled",false);
 }
</script>
<style>
