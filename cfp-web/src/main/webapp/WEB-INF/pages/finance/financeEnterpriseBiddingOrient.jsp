			...
			<div class="loanlist-bot">
				<div class="loanlist-bot-le">
					<div class="loanlist-bot-le-tp money">
							<div class="loanlist-bot-div">预期年化收益<br /><big data-pay="${loanApplicationListVO.annualRate}">${loanApplicationListVO.annualRate}<small>%</small>
				...
				<div class="loanlist-bot-ri">
					...
					<c:if test="${empty sessionScope.currentUser || userCount eq '0'}">
...
<script type="text/javascript">
    $(function () {
    	...
    	<c:if test="${not empty sessionScope.currentUser && userCount eq '0'}">
	    	$(".loanlist-bot-ri input.ipt-input[type=text]:visible,.loanlist-bot-ri button").each(function() {
	          $(this).attr("disabled",true);
	        });
	    </c:if>
...
<div class="masklayer1" id="bigpicture2" style="display: none;">
 ...
 <input value="${empty sessionScope.currentUser || userCount eq '0'}"  type="hidden" id="iTrueLogin" />
</div>
<!-- masklayer end -->
<!--飘窗登录前start-->
 <div class="bay-window" id="bay-window">
      ...
      <input value="${userCount eq '0'}"  type="hidden" id="iTrueLogin" />
</div>
<!--飘窗end-->
<script>
//当滚动条的位置处于距顶部100像素以下时，跳转链接出现，否则消失
$(function () {
	  var iTrue=  $("#iTrueLogin").val();
