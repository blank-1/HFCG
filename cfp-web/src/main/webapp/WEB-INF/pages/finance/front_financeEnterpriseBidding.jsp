...
<div class="news_con">
	<div class="news_crumbs">财富派首页  &gt;  我要理财  &gt;  
				...
				<div class="loanlist-bot-ri">
					...
					<p>剩余金额：<span id="mony2" data-money="${loanApplicationListVO.remain}">
	                    ...
	                    <c:if test="${empty sessionScope.currentUser}">
	                        <font><a href="javascript:" style="color:#35B0EB;padding:0 5px;" data-mask='mask' data-id="login" disabled="true">登录</a><a>查看</a></font>
	                    </c:if>
						</p>
					<c:if test="${not empty sessionScope.currentUser}">
					...
					<p class="mt-20">
						<span class="list-inpit">
							<input type="text" id="money" style="margin-bottom:5px;" placeholder="请输入投标金额" maxlength="10" class="ipt-input" disabled="true"/><em class="yuan1">元</em>
						</span>
						<span class="list-btn" id="p_button">
							...
							<c:if test="${empty sessionScope.currentUser}">
								<button type="button" data-mask='mask' data-id="login" disabled="true">
									提交订单，去支付
								</button>
							</c:if>
						</span>
					...
					<p class="mt-20">
						<span class="list-toub">
							<button type="button" id="firstfinan" class="btn-small" disabled="true">最大可投</button>
							<button type="button" id="allbalance" disabled="true" <c:if test="${empty sessionScope.currentUser}">data-mask='mask' data-id="login"</c:if> class="btn-big">全部余额</button>
						</span>
...
<div class="list_tab">
    <h6>
		<c:if test="${loanApplicationListVO.loanType eq '2'}">
            ...
            <span class="border-right-none">投标信息</span>
	    </c:if>
...
<div class="masklayer masklback" id="cjd">
    ...
    <div class="flbuy">
        ...
        <p class="clearFloat"><span class="fsp1">预期收益：</span> <span class="fsp2"><small id="expected" class="c_red">
            300.05
        </small>元</span></p>
        ...
        <form action="${ctx}/finance/bidLoanByAccountBalance" id="lendForm" method="post" name="form" class=" mt-20">
            ...
            <div class="groupbtn clearFloat">
                ...
                <label>
                    <button type="button" id="otherPay" class="btn btn-write ml-20 mt-0 help-tips">使用其它方式支付<p><img src="${ctx}/images/cfq_icon.jpg" />戳我，戳我，使用优惠券吧~</p></button>
                </label>
            </div>
        </form>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < d_rows.length; i++) {
                    ...
                    thtml += '<tr>';
                    thtml += '<td style="font-size:12px!important">' + data.lenderName + '</td>';
                    thtml += '<td style="font-size:12px!important">' + fmoney(data.lendAmount, 2) + '</td>';
                    thtml += '<td style="font-size:12px!important">' + dateTimeFormatter(data.lendTime) + '</td>';
                    thtml += '</tr>';
                ...
                <div class="thumbCont">
                    <ul>
                        ...
                        <c:forEach items="${customerUploadSnapshots}" var="customersnapshot">
                            <li>
                                <div><img src="${basePath}${customersnapshot.attachment.thumbnailUrl}" url="url"
                                          bigImg="${basePath}${customersnapshot.attachment.url}"
