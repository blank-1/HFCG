	...
	$.movingBoxes = function(el, options){
		...
		base.init = function(){
			...
			base.$left = base.$wrap.find('.mb-left').click(function(){
				$("#lrtk li.current").prev().removeClass("phide").siblings().addClass("phide");
				base.goBack();
				
				return false;
			});
			base.$right = base.$wrap.find('.mb-right').click(function(){
				$("#lrtk li.current").next().removeClass("phide").siblings().addClass("phide");
				base.goForward();
				return false;
			});
		...
		base.change = function(curPanel, callback, flag){
			...
			if (o.wrap) {
				...
				$("#text"+curPanel).show();
				$("#text"+(curPanel-1)).hide();
				$("#text"+(curPanel-2)).hide();
				$("#text"+(curPanel-3)).hide();
				$("#text"+(curPanel-4)).hide();
				$("#text"+(curPanel-5)).hide();
				$("#text"+(curPanel-6)).hide();
				$("#text"+(curPanel-7)).hide();
				$("#text"+(curPanel-8)).hide();
				$("#text"+(curPanel-9)).hide();
				$("#text"+(curPanel-10)).hide();
				$("#text"+(curPanel-11)).hide();
				$("#text"+(curPanel+1)).hide();
				$("#text"+(curPanel+2)).hide();
				$("#text"+(curPanel+3)).hide();
				$("#text"+(curPanel+4)).hide();
