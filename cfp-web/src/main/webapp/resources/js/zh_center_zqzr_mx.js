...
function financeHtml(page,rows,flist,ylist){
	...
	$("#pre_ul_big").html("");
	$("#pageList").html("");
	$("#pre_ul_big").html('<div class="tcdPageCode"></div>');
	//组织参数
//	var creditorRightsStatus = "";
...
//	});
	...
	$.ajax({
		url:rootPath+"/finance/getTurnCreditRightList",
		type:"post",
		data: {
			"pageSize": rows,
			"pageNo": page,
...
function render(flist,ylist){
	...
	for(var i=0;i<flist.length;i++){
		...
		if (flist[i][19] != undefined && flist[i][19] != null && flist[i][19].length>0) {
			...
			for(var x=0;x<rateOrderList.length;x++){
				...
				if(rorder.rateType == '1'){
					//加息券
					rateValue = "<i class='borlisdeta2'>券</i>您已使用" + rorder.rateValue
					+ "%加息券";
				}
		...
		thtml +='<li class="cjzq_list_hide">'+flist[i][8]+'</li>';
		...
		'<dl class="dl_box2">'+
        '<dd>借款时长：'+(vo__.dueTimeType == 1? vo__.cycleCount * vo__.dueTime: vo__.cycleCount) +(vo__.dueTimeType == 1?'天':'个月')+'</dd>'+
		...
		thtml+='</dd>'+
		'<dd>还款方式：'+flist[i][11].repayMentMethod+'</dd>'+
		'</dl>'+
		'<dl class="dl_box4">'+
		'<dd>'+ '<img src="../images/lc_xq_05.jpg"><a target="_blank" href="' + rootPath + "/finance/downloadAgreement?creditorRightsId=" + flist[i][13] + '">协议下载</a>'
		'</dd>';
		if(flist[i][6] == "已生效" && flist[i][16] == true){
			thtml+=	'<dd><img src="../images/cj_mx_04.jpg"><a href="' + rootPath + "/finance/turnCreditRightBidding?creditorRightsId=" + flist[i][13] +'" style="color:#f66;">债权转让</a></dd>';
		}
		thtml+= '</dl></div>';
		if (flist[i][17] == 1 || (flist[i][17] == 2 && flist[i][14] != '放款')) {// 债权转让奖励判断
			thtml += '<p class="pborlde">' + award + '</p>';
		}
		if (flist[i][19] != undefined && flist[i][19] != null && flist[i][19].length>0 && rateValue!=null) {// 出借债权加息或活动奖励标记
	    	thtml += '<p class="pborlde">' + rateValue + '</p>';
	    }
		thtml +='<div class="div_show_bot">';
		thtml +='<ul class="div_show_bot_ultop"><li class="zqzr_list_hide">回款期</li><li>回款日期</li><li>应回本金(元)</li><li>应回利息(元)</li><li class="cjzq_list_hide">罚息(元)</li><li class="div_show_bot_ul2_120 cjzq_list_hide">应回款总额(元)</li><li class="div_show_bot_ul2_120 cjzq_list_hide3">已回款总额(元)</li><li class="cjzq_list_hide">应缴费用(元)</li> <li class="div_show_bot_ul2_60">状态</li></ul>';
		thtml +='<ul class="div_show_bot_ul">';
		
		for(var j=0; j<ylist[i].length; j++){
			thtml +='<li>';
			thtml +='<ul class="div_show_bot_ul2">';
			thtml +='<li class="zqzr_list_hide">'+ylist[i][j][0]+'</li>';
			thtml +='<li>'+ylist[i][j][1]+'</li>';
			thtml +='<li>'+ylist[i][j][2]+'</li>';
			thtml +='<li>'+ylist[i][j][3]+'</li>';
			thtml +='<li class="zqzr_list_hide">'+ylist[i][j][4]+'</li>';
			thtml +='<li class="div_show_bot_ul2_120 zqzr_list_hide">'+ylist[i][j][6]+'</li>';
			thtml +='<li class="div_show_bot_ul2_120 cjzq_list_hide3">'+ylist[i][j][7]+'</li>';
			thtml +='<li  class="zqzr_list_hide">'+ylist[i][j][5]+'</li>';
