...
//全屏滚动效果
	function swiper(){
		var winWid = $("body").width(),
			leg = $(".l_sections li").length,
			left, startX,startY,X,Y;
		// console.log((leg-1)*winWid);

		// 监听touchmove事件
		$(".l_sections").on("touchstart", function(e) {
		    e.preventDefault();
		    e.stopPropagation();
		    startX = e.originalEvent.changedTouches[0].pageX,
		    startY = e.originalEvent.changedTouches[0].pageY;
		    left1 = $(".l_sections").offset().left;
		});
		$(".l_sections").on("touchmove", function(e) {
		    e.preventDefault();
		    e.stopPropagation();
		    ...
		    X = moveEndX - startX,
		    Y = moveEndY - startY;
		    $(".l_sections ").css("left",left1+X);
		});
		$(".l_sections").on("touchend", function(e) {
		    e.preventDefault();
		    e.stopPropagation();
		    moveEndX = e.originalEvent.changedTouches[0].pageX,
		    moveEndY = e.originalEvent.changedTouches[0].pageY,
		    ...
		    // console.log(X2);
		    // console.log(left1);
		    left2 = $(".l_sections").offset().left;
		    console.log(left2);
		if (Math.abs(X) < winWid/2 && X2!=0 && X!=0) {
			$(".l_sections").css("left",left2-X);
		}else if(Math.abs(X) >= winWid/2 &&X>0&& X2!=0){ 
			if (left1 == 0) {
	  			$(".l_sections").css("left",0);
	  		}else{
	  			$(".l_sections").css("left",left1+winWid);
	  		}
	  	}else if(Math.abs(X) >= winWid/2 &&X<0&& X2!=0){
	  		if (left1 == -(leg-1)*winWid) {
	  			$(".l_sections").css("left",-(leg-1)*winWid);
	  		}else{
	  			$(".l_sections").css("left",left1-winWid);
	  		}
	  		...
	  		if (X2==0) {
	  			...
	  			// alert(legEq)
	  			location.href = $(".l_sections li:eq("+ legEq +")").attr("data-href");
	  		};
	  		return false;
		}	  	
		});
