...
function addDom(dateY,dateM) {
    //console.log('月：' + dateY+"("+typeof dateY+")"+dateM+"("+typeof dateM+")");
    
    // 翻页后删除节点及解绑委托事件-------start
    $(".l_infoPop").parent().off("click");
    $(".l_infoPop").remove();
    $(".l_infoList").children().remove();
    $(".l_infoBox").addClass("l_noData")
    // 翻页后删除节点及解绑委托事件-------end
    
    /********************
    *json键说明
    *"monthMoney":当月总回款额
    *"dayList":当月有回款记录的日期数组
    *"day":当月有回款记录的日子
    *"leg":当日回款记录的个数
    *
    **********************/
    ...
    $.ajax({
	    url:urlBase+'/finance/getPaymentCalendarMonthData?date='+dateY+'-'+dateM,
	    type:"POST",
	    success:function(data){
	        if(data.result == 'success'){
	        	var l_data = data.data;
	        	
//	        	var l_data = { "monthMoney":"1234567.88", "dayList":[{"day":3,"leg":2},{"day":4,"leg":5},{"day":18,"leg":3},{"day":20,"leg":2},{"day":28,"leg":6}]};
	        	//console.log(JSON.stringify(l_data));
	        	// 遍历json并插入相应dom元素，以及元素的点击事件。
	            $.each(l_data,function() {
	                $("#l_money").text(l_data.monthMoney);
	                for (var i = 0; i < l_data.dayList.length; i++) {
	                  $(".tables td").each(function(ind) {
	                    var day = l_data.dayList[i].day;
	                    if($(this).attr('data_m') == dateM && $(this).attr('data_d') == day ){
	                      var span = $("<span>").addClass("l_infoPop").text(l_data.dayList[i].leg).appendTo($(this));
	                    }
	                  })
	                }
	            });
	            
	            $(".l_infoPop").parent().on("click",function(e) {
	            	e.stopPropagation();
	              
	              var YY = $(this).attr('data_y'),
	                  MM = $(this).attr('data_m'),
	                  DD = $(this).attr('data_d');
	                  //console.log('天：' + YY+"("+typeof YY+")"+MM+"("+typeof MM+")"+DD+"("+typeof DD+")")
	              $(".l_infoBox").addClass("l_loadingData");
	              ...
	              $(".l_infoList").children().remove();
	                ...
	                *"moneyDay":当日总回款额
	                *"Daylist":当日回款记录列表数组
	                *"pjType":当前列表项标的种类，参考pjType方法添加相应css
	                *"benOrLi":当前列表项的金额为利息还是本金
	                *"timerAll":当前列表项的总期数
	                *"timmerNow":当前列表项的当前期数
	                *"backMoney":当前列表项的回款金额（或利息金额）
	                *"limitTime":当前列表项的到期时间
	                *"hasBackOrNot":当前列表项是否为已回款
	                *
	                **********************/
	              
	              $.ajax({
	          	    url:urlBase+'/finance/getPaymentCalendarDayData?date='+YY+'-'+MM+'-'+DD,
	          	    type:"POST",
	          	    success:function(data){
	          	        if(data.result == 'success'){
	          	        	var l_listData = data.data;
	          	        	
	          	        //var l_listData = { "moneyTime":3, "moneyDay":"2204.32","Daylist":[{"pjType":0,"title":"F17033114103-6-1440万个人房抵借款标第6期","benOrLi":0,"timerAll":12,"timmerNow":1,"backMoney":"123.12","limitTime":"2017.06.20","hasBackOrNot":0},{"pjType":0,"title":"F17033114103-6-1440万个人房抵借款标第6期","benOrLi":1,"timerAll":12,"timmerNow":1,"backMoney":"3456.12","limitTime":"2017.06.20","hasBackOrNot":0},{"pjType":1,"title":"F17033114103-6-1440万个人房抵借款标第6期","benOrLi":0,"timerAll":12,"timmerNow":1,"backMoney":3467.12,"limitTime":"2017.06.20","hasBackOrNot":1}]};
	      	              
	      	              $.each(l_listData,function(inx,obj) {
	      	                $(".l_infoBox").removeClass("l_noData");
	      	                $("#legDay").text(l_listData.moneyTime);
	      	                $("#moneyDay").text(l_listData.moneyDay);
	      	                if (inx == "Daylist") {
	      	                  for (var i = 0; i < obj.length; i++) {
	      	                    var li = $("<li>").addClass(hasBackOrNot(obj[i].hasBackOrNot)).appendTo($(".l_infoList")),
	      	                        h2 = $("<h2>").addClass("text-overflow "+pjType(obj[i].pjType)).text(obj[i].title).appendTo(li),
	      	                        p1 = $("<p>").html("<span class='"+benLi(obj[i].benOrLi)+"'></span><span id='timer'>"+obj[i].timmerNow+"/"+obj[i].timerAll+"</span>").appendTo(li),
	      	                        p2 = $("<p>").html("<span id='benOrLi'>"+obj[i].backMoney+"</span><span>回款金额</span>").appendTo(li),
	      	                        p3 = $("<p>").html("<span id='limitTime'>"+obj[i].limitTime+"</span><span>到期时间</span>").appendTo(li);
	      	                  }
	      	                }
	      	            ...
	      	            $(".l_infoBox").removeClass("l_loadingData");
	        	        }else{
	        	        	alert(data.errMsg);
	        	        }
	        	    }
...
function jump(yyyy, mm, dd) {
    ...
    $("#ymym").html(sev_y + "年" + sev_m + "月");
    $(".covers").hide();
    mySwiper.slideTo(2, 500, addDom(sev_y,sev_m));
    mySwiper.unlockSwipes();
...
$(function() {
	  setTimeout(function() {
		    addDom($('#ymym').text().slice(0,4),$('#ymym').text().slice(-2,-1));
		  },10)
