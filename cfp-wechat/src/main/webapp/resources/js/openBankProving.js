...
require(['jquery','swal'],function($,swal) {
	$(function() {
        ...
        let Verification = {
            _init(){//自定义函数
                $("#NextStep").on("click",Verification.checkOutPwd);
				$("#UnAgree").on("click",Verification.AgreeSelete)
            },
            checkOutPwd(){
                let pwdVal = $("#user").val(),
                    againPwdVal = $("#pwd").val();
                if(pwdVal =="" || againPwdVal ==""){
                    swal(
                        '密码不能为空',
                        '',
                        'warning'
                ...
                if(pwdVal.length < 6 && pwdVal > 16 || againPwdVal.length < 6 && againPwdVal > 16){
                     swal(
                        '密码必须大于4位数字和小于16位数字',
                        '',
                        'warning'
        ...
        function aa() {
            ...
            setTimeout(function () {
                ...
                var IDVal = $("#id-case").val();
                var mobileVal = $("#mobile-case").val();
                ...
                $.ajax({
                    ...
                    type: "POST",   //请求方式
                    data: $('input', '.param_box').serialize(),
